...
create_map_report <- function(input.seq, CountsFrom, SNPCall, GenoCall){
  if(length(input.seq$seq.num) > 60){
    ...
    size <- length(input.seq$seq.num)/2
    overlap <- size/2
    around <- size/3
  }
  ...
  ggsave(p, filename = paste0(SNPCall, "_", CountsFrom, "_", GenoCall,".png"))
  
  times_df <- data.frame(SNPCall,CountsFrom, GenoCall, time_par[3])
  write_report(times_df, paste0(SNPCall, "_", CountsFrom, "_", GenoCall,"_times.txt"))
  ...
  write_report(sizes_df, paste0(SNPCall, "_", CountsFrom, "_", GenoCall,"_map.txt"))
}
...
# check depths
create_depths_profile(df, vcf, parent1 = parent1, parent2 = parent2, vcf.par = "AD", 
                      recovering = FALSE, GTfrom = "vcf", alpha=0.1)

## FILTERS REPORT
out_name <- paste0(CountsFrom, "_", method_name, "_filters_dfAndGQ.txt")
filters_tab <- create_filters_report(df, CountsFrom, SNPCall=method_name, GenoCall="df")
write_report(filters_tab[[1]], out_name)

lgs <- group(filters_tab[[2]])
lg1 <- make_seq(lgs,1)

create_map_report(input.seq = lg1, CountsFrom = CountsFrom, SNPCall = method_name, GenoCall="df")
...
filters_tab <- create_filters_report(aval.gq, CountsFrom, SNPCall=method_name, GenoCall="df")

lgs <- group(filters_tab[[2]])
lg1 <- make_seq(lgs,1)

create_map_report(lg1, CountsFrom = CountsFrom, SNPCall = method_name, GenoCall="GQ")
...
for (metodology in names(metodologies)){
  ...
  ## Maps
  create_map_report(input.seq = lg1, CountsFrom = CountsFrom, SNPCall = method_name, GenoCall= metodology)
}
